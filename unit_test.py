...
class TestBoardGeneration(unittest.TestCase):
    ...
    def test_game_mode(self):
        controller = game_engine.SolitaireGameController(
            game_engine.SolitaireGame(board_type=game_engine.BoardType.HEXAGON, board_size=7)
        )

        manual_mode = game_engine.ManualGameMode(controller)
        automated_mode = game_engine.AutomatedGameMode(controller)

        self.assertIsInstance(manual_mode, game_engine.GameMode)
        self.assertIsInstance(automated_mode, game_engine.GameMode)


class TestStartNewGame(unittest.TestCase):
    """AC 3.1: start a new game of chosen size/type/mode."""
    ...
    def test_start_new_game(self):
        ...
        self.assertEqual(controller.game.board_type, game_engine.BoardType.ENGLISH)
        self.assertEqual(controller.game.board_size, 7) 
        self.assertEqual(len(controller.game.board_layout), 7)

        automated_mode.start_new_game(board_type=game_engine.BoardType.DIAMOND, board_size=9)
        self.assertEqual(controller.game.board_type, game_engine.BoardType.DIAMOND)
        self.assertEqual(controller.game.board_size, 9)
        self.assertEqual(len(controller.game.board_layout), 9)
...
class TestMoves(unittest.TestCase):
    ...
    def test_make_move_in_automated_game(self):
        ...
        before = _count_pegs(controller.board_layout)
        moved = automated_mode.make_move()
        after = _count_pegs(controller.board_layout)
...
class TestGameOverAndScoring(unittest.TestCase):
    ...
    def test_automated_game_is_over(self):
        game = game_engine.SolitaireGame(board_type=game_engine.BoardType.ENGLISH, board_size=1)
        controller = game_engine.SolitaireGameController(game)
        automated_mode = game_engine.AutomatedGameMode(controller)

        steps = automated_mode.play_until_game_over(max_steps=20)
